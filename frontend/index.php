    ...
    <nav class="bg-white border-b border-slate px-8 py-5 mb-8 flex items-center justify-between shadow-sm">
        ...
        <?php if ($view !== 'dashboard'): ?>
            <a href="?view=dashboard"
                class="text-sm font-medium text-slate-500 hover:text-slate-800 transition-colors">&larr; Back to
                Dashboard</a>
        <?php endif; ?>
    </nav>
    ...
    <main class="max-w-5xl mx-auto px-8">
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate">
                        <?php foreach ($contractors as $c): ?>
                            <?php
                            $badgeColor = '';
                            if ($c['health_status'] === 'Prime')
                                $badgeColor = 'bg-emerald-100 text-emerald-800 border-emerald-200';
                            else if ($c['health_status'] === 'Amber')
                                $badgeColor = 'bg-amber-100 text-amber-800 border-amber-200';
                            else if ($c['health_status'] === 'Toxic')
                                $badgeColor = 'bg-rose-100 text-rose-800 border-rose-200';
                            ?>
                            <tr class="hover:bg-slate-50 transition-colors cursor-pointer group"
                                onclick="window.location.href='?view=passport&id=<?= $c['id'] ?>'">
                                ...
                                <td class="py-5 px-6">
                                    <span
                                        class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border <?= $badgeColor ?>">
                                        <?= htmlspecialchars($c['health_status']) ?>
                                        (<?= htmlspecialchars($c['health_score']) ?>%)
                                    </span>
                                </td>
                                <td class="py-5 px-6 text-right">
                                    <span
                                        class="text-blue-600 group-hover:text-blue-800 font-medium text-sm transition-colors">View
                                        Passport &rarr;</span>
                                </td>
                            </tr>
                        <?php endforeach; ?>
        ...
        <?php elseif ($view === 'passport' && $contractor): ?>
            ...
            <div id="loading-state" class="flex flex-col items-center justify-center py-32">
                <svg class="animate-spin -ml-1 mr-3 h-10 w-10 text-blue-600 mb-6" xmlns="http://www.w3.org/2000/svg"
                ...
                <div id="loading-text" class="text-lg font-medium text-slate-600 transition-opacity duration-300">Syncing
                    LHDN e-Invoice API...</div>
            </div>
            ...
            <div id="loaded-state" class="hidden space-y-8 animate-fade-in pb-12">
                ...
                <div class="flex justify-between items-end mb-4 border-b border-slate pb-6">
                    <div>
                        <h1 class="text-3xl font-bold tracking-tight text-slate-900">
                            <?= htmlspecialchars($contractor['name']) ?></h1>
                        <p class="text-slate-500 mt-2 text-lg">Accounts Receivable: <span
                                class="font-medium text-slate-700">RM
                                <?= htmlspecialchars($contractor['owe_amount']) ?></span></p>
                    </div>
                    <?php
                    $badgeColor = '';
                    if ($contractor['health_status'] === 'Prime')
                        $badgeColor = 'bg-emerald-100 text-emerald-800 border-emerald-200';
                    else if ($contractor['health_status'] === 'Amber')
                        $badgeColor = 'bg-amber-100 text-amber-800 border-amber-200';
                    else if ($contractor['health_status'] === 'Toxic')
                        $badgeColor = 'bg-rose-100 text-rose-800 border-rose-200';
                    ?>
                    <div class="text-right">
                        <span
                            class="inline-flex items-center px-4 py-1.5 rounded-full text-sm font-bold border <?= $badgeColor ?>">
                            <?= htmlspecialchars($contractor['health_status']) ?> Passport
                            (<?= htmlspecialchars($contractor['health_score']) ?>%)
                        </span>
                    </div>
                </div>
                ...
                <div class="bg-white border border-slate rounded-lg p-8 shadow-sm">
                    <h2 class="text-sm font-bold text-slate-400 border-b border-slate pb-4 mb-6 uppercase tracking-widest">
                        Trade Passport Metrics</h2>
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-8">
                        <div>
                            <div class="text-sm font-medium text-slate-500 mb-1">Transaction Frequency</div>
                            <div class="text-2xl font-bold text-slate-800"><?= htmlspecialchars($contractor['frequency']) ?>
                            </div>
                        </div>
                        <div>
                            <div class="text-sm font-medium text-slate-500 mb-1">Payment Habits</div>
                            <div class="text-2xl font-bold text-slate-800">
                                <?= htmlspecialchars($contractor['payment_days']) ?> Days</div>
                        </div>
                        <div>
                            <div class="text-sm font-medium text-slate-500 mb-1">Years of Cooperation</div>
                            <div class="text-2xl font-bold text-slate-800">
                                <?= htmlspecialchars($contractor['coop_years']) ?> Years</div>
                        </div>
                        <div>
                            <div class="text-sm font-medium text-slate-500 mb-1">Amount Trend</div>
                            <div class="text-2xl font-bold text-slate-800"><?= htmlspecialchars($contractor['trend']) ?>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="bg-white border border-slate rounded-lg p-8 shadow-sm flex flex-col">
                        ...
                        <div class="flex-grow">
                            <?php if ($contractor['health_status'] === 'Prime'): ?>
                                <p class="text-slate-600 mb-6">This contractor has an excellent payment history. Frictionless
                                    processing enabled.</p>
                            <?php elseif ($contractor['health_status'] === 'Amber'): ?>
                                <div
                                    class="bg-amber-50 border border-amber-200 text-amber-900 px-4 py-4 rounded mb-6 text-sm flex items-start">
                                    <svg class="h-5 w-5 text-amber-600 mr-3 flex-shrink-0 mt-0.5" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z">
                                        </path>
                                    </svg>
                                    <span><strong>Late Payer History:</strong> This contractor has a history of delayed
                                        payments. Proceed with standard caution.</span>
                                </div>
                            <?php elseif ($contractor['health_status'] === 'Toxic'): ?>
                                <div
                                    class="bg-rose-50 border border-rose-200 text-rose-900 px-4 py-4 rounded mb-6 text-sm flex items-start">
                                    <svg class="h-5 w-5 text-rose-600 mr-3 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24"
                                        stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z">
                                        </path>
                                    </svg>
                                    <span><strong>High Default Risk:</strong> This contractor exhibits severe late payment
                                        behaviors and declining trends.</span>
                                </div>
                                <label class="flex items-start space-x-3 mb-6 cursor-pointer group">
                                    <input type="checkbox" id="toxic-confirm"
                                        class="w-5 h-5 mt-0.5 text-slate-900 rounded border-slate-300 focus:ring-slate-900 cursor-pointer">
                                    <span class="text-sm text-slate-600 group-hover:text-slate-800 select-none leading-tight">I
                                        acknowledge the high risk of default and authorize this delivery order despite the Toxic
                                        rating.</span>
                                </label>
                            <?php endif; ?>
                        </div>
                        ...
                        <?php if ($contractor['health_status'] === 'Toxic'): ?>
                            <button id="toxic-btn" disabled
                                class="mt-auto w-full bg-slate-200 text-slate-400 font-bold py-3.5 px-4 rounded cursor-not-allowed transition-all duration-200 border border-transparent">
                                Print Delivery Order
                            </button>
                        <?php else: ?>
                            <button
                                class="mt-auto w-full bg-slate-900 hover:bg-slate-800 text-white font-bold py-3.5 px-4 rounded transition-colors focus:outline-none focus:ring-2 focus:ring-slate-900 focus:ring-offset-2 border border-transparent shadow-sm">
                                Print Delivery Order
                            </button>
                        <?php endif; ?>
                    </div>
                    ...
                    <!-- The CapBay Bridge -->
                    <div
                        class="bg-slate-50 border border-slate rounded-lg p-8 shadow-sm flex flex-col justify-center items-center text-center">
                        <div class="mb-8">
                            <div
                                class="h-16 w-16 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center mx-auto mb-4">
                            ...
                            <p class="text-slate-500 max-w-xs mx-auto">Need immediate cash flow for this invoice? Submit
                                directly to our financing partners.</p>
                        </div>
                        <button id="capbay-btn"
                            class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 px-4 rounded transition-colors focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 shadow-sm">
                            Submit AR to CapBay
                        </button>
                    </div>
                </div>
            ...
            <div id="capbay-modal"
                class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center hidden opacity-0 transition-opacity duration-300 z-50">
                <div
                    class="bg-white rounded-xl p-8 max-w-md w-full shadow-2xl transform scale-95 transition-all duration-300 mx-4 border border-slate">
                    ...
                    <p class="text-slate-600 text-center mb-8 leading-relaxed">Accounts Receivable and Trade Passport
                        successfully submitted to CapBay for institutional financing.</p>
                    <button id="close-modal-btn"
                        class="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold py-3.5 px-4 rounded transition-colors focus:outline-none focus:ring-2 focus:ring-slate-900 focus:ring-offset-2">
                        Return to Dashboard
                    </button>
                </div>
            </div>
